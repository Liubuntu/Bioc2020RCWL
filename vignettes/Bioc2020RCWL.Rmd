...
```{r}
library(git2r)
path <- file.path(tempfile(pattern="git2r-"), "testdata")
dir.create(path, recursive=TRUE)
repo <- clone("https://github.com/hubentu/testdata.git", path)
dir(path)
```
...
system.time(
    runCWL(cwl = STARindex, outdir = file.path(path, "STARindex_output"), showLog = TRUE)
    ) ## elapsed: 74.795S
dir(file.path(path, "STARindex_output"), recursive = TRUE)
```
...
runCWL(STARsolo, outdir = file.path(path, "STARsolo_output"), showLog = TRUE)  ## approximately 2 mins
dir(file.path(path, "STARsolo_output"), recursive = TRUE)
```     
...
inputs(DropletUtils)
DropletUtils$dirname <- file.path(path, "STARsolo_output/Solo.out")
DropletUtils$lower <- 100
DropletUtils$df <- 5
runCWL(DropletUtils, outdir = file.path(path, "dropletUtils_output"), showLog = TRUE)
dir(file.path(path, "dropletUtils_output"))
```
...
sce <- readRDS(file.path(path, "dropletUtils_output/sce_filtered.rds"))
sce
```
...
STARsoloDropletUtils$genomeDir <- file.path(path, "STARindex_output/STARindex")
STARsoloDropletUtils$whiteList <- cblist
STARsoloDropletUtils$runThreadN <- 2
runCWL(STARsoloDropletUtils, outdir = "STARsoloDropletUtils_output", showLog = TRUE)
```

Let's take a look at the output for this pipeline: 
