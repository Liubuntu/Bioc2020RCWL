...
standard for the development of data analysis workflows that is
...
easy customization of complex bioinformatics analysis. Here we
included a scRNAseq preprocessing pipeline that uses `STARsolo` for
alignment and quantification, and `DropletUtils` for filtering raw
gene-barcode matrix and removing empty droplets. This pipeline
demonstrates the typical use case of our packages. More details for
usage and examples are available on Rcwl website:
...
10x genomics, consisting of 1000 Peripheral blood mononuclear cells
...
Since CWL doesn't support the integration of _R_ packages or _R_
function, and lacks interoperability of very widely used _R_ tools for
downstream data analysis. 

The support for integration of _R_ functions in `Rcwl` is a very
unique feature, where we can easily connect the upstream data
preprocessing steps (previously based on command line tools) and the
...
the output files from previous step and convert into a
...
`DropletUtils` for a streamlined pre-processing analysis within _R_.
...
development, and we welcome any question for the functionalities and
...
We are trying to make this project as a community effort for
developing and sharing of specific sets of tools and pipelines in
their bioinformatics domains. Please feel free to make your pull
requests for your recipes
[here](https://github.com/hubentu/RcwlRecipes) and we are happy to add
