...
scRNA-seq outputs it produces (and requires much configuration to run
...
programmatically in _R_ (the other vignette called "Bioc2020CWL4devel"
covers more of this topic for developers). For usage of more than a
hundred pre-built and tested tools/pipelines in `RcwlPipelines`, all
we need to do is as simple as: 1) Search and load the tools/pipelines,
2) assign values for each of the defined parameters, 3) execute the
command. All steps are done programmatically in _R_, and then we can
get the results ready in the user-specified directory.
...
writeCWL(STARindex, prefix = file.path(outpath, "STARindex"))
```
...
"STARindex_output/STARindex" under `outpath`. These results are ready to pass to
...
In a pipeline, we only need to assign input values for the whole pipeline, not individual tools involved. The input and output between each steps are pre-defined in the pipeline to ensure a smooth passing.  
